...
def listar_archivos(db: Session, filtros: dict):
    """
    Lista archivos (originales o modificados). Devuelve la lista de filas de la DB.
    """
    ...
    try:
        ...
        if filtros.get("q"):
            ...
            base_sql += (" AND (nombre_archivo LIKE :q OR responsable LIKE :q "
                         "OR observacion LIKE :q OR categoria LIKE :q OR codigo_sgps LIKE :q "
                         "OR nombre_centro LIKE :q OR regional LIKE :q)")
        ...
        # Cambio: query -> consulta_final
        consulta_final = text(base_sql)
        resultados = db.execute(consulta_final, parametros_sql).mappings().all()
        return resultados
    except SQLAlchemyError as error_sql:
